...
// launch server at tests' start
// so that you do not need to have
// server running while launching tests
...
public class AccountClientTests {
	...
	public void getAccount() {
		...
		// - Run the test and ensure that it passes.
		URI uri = URI.create(BASE_URL + port + "/accounts/0");
	...
	public void addAndDeleteBeneficiary() {
		...
		String url = BASE_URL + port + "/accounts/{accountId}/beneficiaries";
		URI newBeneficiaryLocation = restTemplate.postForLocation(url, "David", 1);
		...
		HttpClientErrorException httpClientErrorException = assertThrows(HttpClientErrorException.class, () -> {
			...
			//   was not successful.
			Beneficiary notFound = restTemplate.getForObject(newBeneficiaryLocation, Beneficiary.class);
		});
